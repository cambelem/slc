...
<pre>';
        ...
        case version_compare($current_version, '3.0.2', '<'):
            $db = new PHPWS_DB();
            $result = $db->importFile(PHPWS_SOURCE_DIR . 'mod/slc/boost/updates/update_3_0_2.sql');
            if (PEAR::isError($result)) {
                return $result;
            }
        case version_compare($current_version, '3.0.3', '<'):
            $db = new PHPWS_DB();
            $result = $db->importFile(PHPWS_SOURCE_DIR . 'mod/slc/boost/updates/update_3_0_3.sql');
            if (PEAR::isError($result)) {
                return $result;
            }
        case version_compare($current_version, '3.0.4', '<'):
            $db = new PHPWS_DB();
            $result = $db->importFile(PHPWS_SOURCE_DIR . 'mod/slc/boost/updates/update_3_0_4.sql');
            if (PEAR::isError($result)) {
                return $result;
            }
    }
